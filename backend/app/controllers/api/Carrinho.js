...
const Carrinho = {
    ...
    adicionarQuantidadeCarrinho: async (req, res) => {
        console.log(req.body);
        const produtoCor = req.body.cor;
        const produtoNome = req.body.nome;
        const produtoTamanho = req.body.tamanho;
        const produtoId = req.body.id;
        const quantidade = req.body.quantidade;
        const estoqueNoBD = req.body.estoqueProduto
        ...
        for (let index = 0; index < req.session.carrinho.length; index++) {
            ...
            if (req.session.carrinho[index].id == produtoId && req.session.carrinho[index].nome == produtoNome && req.session.carrinho[index].tamanho == produtoTamanho && req.session.carrinho[index].cor == produtoCor) {
                ...
                req.session.carrinho[index].quantidade = req.session.carrinho[index].quantidade + quantidade;
                req.session.save();
                if (req.session.carrinho[index].quantidade > estoqueNoBD) {
                    req.session.carrinho[index].quantidade = 1;
                    req.session.save();
                }
                if (req.session.carrinho[index].quantidade < 1) {
                    req.session.carrinho[index].quantidade = estoqueNoBD;
                    req.session.save();
                }
                ...
                console.log("Carrinho quantidade atualizada: ", req.session.carrinho);
